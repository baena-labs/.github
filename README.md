...
- **`reusable-sync-labels.yml`** is **additive only** — it creates missing labels and updates color/description on existing matches, but never deletes labels. Repo-specific labels (e.g. `area/cases` in `legal-brain`) survive untouched. Edit `labels.yml` in this repo to update the canonical set; consumers re-sync on `workflow_dispatch` or whatever trigger they configure on their caller. Callers must grant `permissions: issues: write`. The label naming convention (slash-style families: `priority/`, `type/`, `area/`, `status/`) is decided in `baena-labs/org-config` issue 16; new families land here as that taxonomy is finalized.
